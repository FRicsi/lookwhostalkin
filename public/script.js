...
document.addEventListener("DOMContentLoaded", () => {
  console.log("DOM ready, MODE =", MODE);
  ...
  if (MODE === "before") {
    ...
    headline.innerHTML =
      "Hamarosan kiderül,<br>kislány leszek-e vagy kisfiú!";
  }
...
if (MODE === "before") {
  ...
  function spawnToy(banner, direction) {
    ...
    // indulási pozíció
    toy.style.left =
      direction === 1 ? `-${SPAWN_OFFSET_VW}vw` : `calc(100% + ${SPAWN_OFFSET_VW}vw)`;
      
      const speedJitter = 0.85 + Math.random() * 0.3; // ±15%
      const durationSec = ((110 + SPAWN_OFFSET_VW * 2) / SPEED_VW_PER_SEC) * speedJitter;
